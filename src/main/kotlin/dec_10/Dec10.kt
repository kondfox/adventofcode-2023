...
fun part2(input: List<String>): Int {
    ...
    val start = nodes.firstNotNullOf { it.firstOrNull { it.pipe == 'S' } }
    val neighbours: List<Node> = neighbours(start, nodes)
    val loop = listOf(start) + findLoop(nodes, neighbours.first())
    val doubleNodes = doubleNodes(nodes)
    val doubleStart = doubleNodes.firstNotNullOf { it.firstOrNull { it.pipe == 'S' } }
    val doubleNeighbours: List<Node> = neighbours(doubleStart, doubleNodes)
    val doubleLoop = listOf(doubleStart) + findLoop(doubleNodes, doubleNeighbours.first())
